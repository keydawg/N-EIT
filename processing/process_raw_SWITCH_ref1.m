   ...
   path_n='E:\GSK\ActCh\P10_sc_disect';
   EIT_fname = 'CR_EIT019_T_EIT_85uA_6kHz_20s_200ms_Prt1_ref1_CRUSHED.vhdr';
   Prt_name='Prt_1.txt';

   map_p=[ 3 4 7 8 9 12 13 14 15 16 19 21 22 23 26 27 28]-1; % Montage
   map_=[ 3 4 7 8 9 12 13 14 15 16 19 21 22 23 26 27 28]; % Actual numbering
   ...
   Noise_thres = 10000;  % Noise threshold (in uV)
...
% Load Prt and check that switches are ok
 ...
 EEG = pop_loadbv('',EIT_fname,[SWITCH(1)+Fs SWITCH(1)+5*Fs]);
 EEG.data=EEG.data(map_p,:);
 V_inj = detrend(double(EEG.data(map_==Prt(1,2),:)'),'constant'); 
 
 [Pxx,w] = pwelch(V_inj,[],[],25000,Fs);
    w_ind = find(w>105);
    [~,maxw] = max(Pxx(w_ind));
    Fc = w(w_ind(maxw));
...
%save([EIT_fname(1:end-4) '.mat'],'EIT','-v7.3');
